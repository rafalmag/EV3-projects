    ...
    public void testPFSingleOutputMode_SubmodePWM() {
        ...
        while (!(Button.ESCAPE.isDown())) {
            ...
            if (Button.ENTER.isDown()) {
                ...
                for (int i = 0; i < 4; i++) {
                    if (toggleBit == 0) {
                        pfIRLink.sendPFSingleOutput(i, NewIRLink.PF_SOM_SUBMODE_PWM, NewIRLink.PF_SOM_OUTPUT_A, NewIRLink.PF_PWM_COMMAND_FLOAT);
                        pfIRLink.sendPFSingleOutput(i, NewIRLink.PF_SOM_SUBMODE_PWM, NewIRLink.PF_SOM_OUTPUT_B, NewIRLink.PF_PWM_COMMAND_FLOAT);
                        LCD.drawString("Float      ", 0, 1);
                    } else {
                        pfIRLink.sendPFSingleOutput(i, NewIRLink.PF_SOM_SUBMODE_PWM, NewIRLink.PF_SOM_OUTPUT_A, NewIRLink.PF_PWM_COMMAND_BRAKEFLOAT);
                        pfIRLink.sendPFSingleOutput(i, NewIRLink.PF_SOM_SUBMODE_PWM, NewIRLink.PF_SOM_OUTPUT_B, NewIRLink.PF_PWM_COMMAND_FLOAT);
            ...
            if (Button.RIGHT.isDown()) {
                ...
                if (pwmSpeed < 7) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        pfIRLink.sendPFSingleOutput(i, NewIRLink.PF_SOM_SUBMODE_PWM, NewIRLink.PF_SOM_OUTPUT_A, pwmSpeed);
                        pfIRLink.sendPFSingleOutput(i, NewIRLink.PF_SOM_SUBMODE_PWM, NewIRLink.PF_SOM_OUTPUT_B, 15 - (pwmSpeed - 1));
