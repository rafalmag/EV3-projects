...
public class EV3ImageMainPanel extends JPanel {
	...
	private int mode = EV3ImageConverter.BIT_8;
	...
	public EV3ImageMainPanel() {
		...
		this.picPanel.addPropertyChangeListener(EV3ImagePicturePanel.IMAGE_UPDATE_PROP, new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				EV3ImageMainPanel.this.updateNxtPart();
			}
		});

		this.codePanel.addPropertyChangeListener(EV3ImageCodePanel.CODE_UPDATE_PROP, new PropertyChangeListener(){
			public void propertyChange(PropertyChangeEvent evt) {
	...
	protected void updateNxtPart() {
		...
		else
		{
			...
			text = EV3ImageConverter.getImageCreateString(currData, currSize,mode);
		}
	...
	protected void updateImageFromCode() {
		...
		try
		{
			image = EV3ImageConverter.getImageFromNxtImageCreateString(code,mode);
			error = null;
		}
		catch (Exception e)
		{
...
//			}
	...
	public JMenuBar getMenuBar(final JPanel panel) {
		...
		menu.addMenuListener(new MenuListener() {
			...
			public void menuSelected(MenuEvent e) {
				boolean b = EV3ImageMainPanel.this.currData != null && EV3ImageMainPanel.this.currSize != null;
	...
	private void importImage(final JPanel panel)
	{
		...
		if (dialog.showOpenDialog(panel) == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				if (image == null) {
					...
					if (image.getWidth() * image.getHeight() > 100 * 64 * 2) {
						...
						canReadImage = JOptionPane.showConfirmDialog(EV3ImageMainPanel.this, message, "Confirm", JOptionPane.YES_NO_OPTION | JOptionPane.WARNING_MESSAGE) == JOptionPane.YES_OPTION;
					}
					if (canReadImage) {
						EV3ImageMainPanel.this.readImage(image);
					}
				}
			} catch (IOException e) {
			...
			if (errorMsg != null) {
				JOptionPane.showMessageDialog(EV3ImageMainPanel.this, errorMsg, "Error", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
	...
	private void saveFile(final JPanel panel)
	{
		...
		if (dialog.showSaveDialog(panel) == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				EV3ImageMainPanel.this.saveImage(file);
			} catch (IOException e) {
	...
	private void openFile(final JPanel panel)
	{
		...
		if (dialog.showOpenDialog(panel) == JFileChooser.APPROVE_OPTION) {
			...
			File file = dialog.getSelectedFile();
			try {
				EV3ImageMainPanel.this.readNxtImage(file);
			} catch (IOException e) {
