...
public abstract class NavigationPanel extends JPanel implements MapApplicationUI, MouseListener, MouseMotionListener, ActionListener {
	...
	protected void popupMenu(MouseEvent me) {
	    ...
	    if (model.getMap() != null) {
	    	boolean inside = model.getMap().inside(new lejos.robotics.geometry.Point((me.getX() / pixelsPerUnit + mapPanel.viewStart.x) , (mapPanel.getHeight() - me.getY())/ pixelsPerUnit + mapPanel.viewStart.y));  
	    	if (!inside) return;
	    }
